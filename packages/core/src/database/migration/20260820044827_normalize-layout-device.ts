...
export default {
  id: "20260820044827_normalize-layout-device",
  up(tx) {
    return Effect.gen(function* () {
      ...
      `)
      yield* tx.run(
        `INSERT INTO \`__new_layout_option\`(\`workspace_id\`, \`user\`, \`style\`, \`device_class\`, \`device_id\`, \`layout_id\`) SELECT \`workspace_id\`, \`user\`, \`style\`, \`device_class\`, COALESCE(\`device_id\`, ''), MIN(\`layout_id\`) FROM \`layout_option\` GROUP BY \`workspace_id\`, \`user\`, \`style\`, \`device_class\`, COALESCE(\`device_id\`, '');`,
      )
