...
Benchmarks do not assert machine-dependent performance budgets. Streaming processes 160 deltas by default and reports renderer-observed completion time, throughput, RAF callback-gap distributions, frame-budget equivalents, long tasks, final geometry, row/Markdown replacement, and SSE delivery state. Delta count and delivery batch are included in result context when overridden. These are main-thread callback diagnostics, not compositor presentation or dropped-frame measurements. Optional visual diagnostics are `null` unless visual profiling is enabled. Assertions require every queued delta to be delivered over one connection with nothing left pending.
